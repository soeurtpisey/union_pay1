...
@class WeCloudStatement;

@interface WeCloudResultSet : NSObject

@property (nonatomic, retain, nullable) WeCloudDatabase *parentDB;
...
/** `WeCloudStatement` used by result set. */

@property (atomic, retain, nullable) WeCloudStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<WeCloudStatement>`
 
 @param statement A `<WeCloudStatement>` to be performed
 
 @param aDB A `<WeCloudDatabase>` to be used
 
 @return A `WeCloudResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(WeCloudStatement *)statement usingParentDatabase:(WeCloudDatabase*)aDB;
