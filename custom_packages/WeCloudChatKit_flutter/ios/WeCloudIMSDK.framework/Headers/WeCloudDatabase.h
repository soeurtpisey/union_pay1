...
#import "WeCloudResultSet.h"

NS_ASSUME_NONNULL_BEGIN

#if ! __has_feature(objc_arc)
    #define WeCloudDBAutorelease(__v) ([__v autorelease]);
    #define WeCloudDBReturnAutoreleased WeCloudDBAutorelease

    #define WeCloudDBRetain(__v) ([__v retain]);
    #define WeCloudDBReturnRetained WeCloudDBRetain

    #define WeCloudDBRelease(__v) ([__v release]);

    #define WeCloudDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define WeCloudDBAutorelease(__v)
    #define WeCloudDBReturnAutoreleased(__v) (__v)

    #define WeCloudDBRetain(__v)
    #define WeCloudDBReturnRetained(__v) (__v)

    #define WeCloudDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details.
    #if OS_OBJECT_USE_OBJC
        #define WeCloudDBDispatchQueueRelease(__v)
    #else
        #define WeCloudDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^WeCloudDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in WeCloudDB are:

 - `WeCloudDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<WeCloudResultSet>` - Represents the results of executing a query on an `WeCloudDatabase`.
 - `<WeCloudDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<WeCloudDatabasePool>` - A pool of `WeCloudDatabase` objects.
 - `<WeCloudStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [WeCloudDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [WeCloudDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `WeCloudDatabase` object and use it across multiple threads. Instead, use `<WeCloudDatabaseQueue>`.
...
@interface WeCloudDatabase : NSObject
...
/** Create a `WeCloudDatabase` object.
 
 An `WeCloudDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `WeCloudDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `WeCloudDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

 WeCloudDatabase *db = [WeCloudDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
 WeCloudDatabase *db     = [WeCloudDatabase databaseWithPath:dbPath];
 ...
 @return `WeCloudDatabase` object if successful; `nil` if failure.
...
/** Create a `WeCloudDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `WeCloudDatabase` connection is closed.
...
/** Initialize a `WeCloudDatabase` object.
...
/** Initialize a `WeCloudDatabase` object.
 ...
 An `WeCloudDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) WeCloudDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<WeCloudResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[WeCloudResultSet next]>`) from one record to the other.
 ...
 @return A `<WeCloudResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see WeCloudResultSet
 @see [`WeCloudResultSet next`](<[WeCloudResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (WeCloudResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (WeCloudResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (WeCloudResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> WeCloudResultSet!`
...
- (WeCloudResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (WeCloudResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (WeCloudResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
+ (NSString*)WeCloudDBUserVersion;

+ (SInt32)WeCloudDBVersion;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [db makeFunctionNamed:@"RemoveDiacritics" arguments:1 block:^(void *context, int argc, void **argv) {
        WeCloudSqliteValueType type = [self.db valueType:argv[0]];
        if (type == WeCloudSqliteValueTypeNull) {
            [self.db resultNullInContext:context];
             return;
        }
        if (type != WeCloudSqliteValueTypeText) {
 ...
 WeCloudResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
typedef NS_ENUM(int, WeCloudSqliteValueType) {
    WeCloudSqliteValueTypeInteger = 1,
    WeCloudSqliteValueTypeFloat   = 2,
    WeCloudSqliteValueTypeText    = 3,
    WeCloudSqliteValueTypeBlob    = 4,
    WeCloudSqliteValueTypeNull    = 5
};


- (WeCloudSqliteValueType)valueType:(void *)argv;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [WeCloudDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one WeCloudDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using WeCloudDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to WeCloudDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using WeCloudDB you will not need to interact directly with `WeCloudStatement`, but rather with `<WeCloudDatabase>` and `<WeCloudResultSet>` only.
...
@interface WeCloudStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
